...
if (DEBUG) {
  M3DebugAdapter = require('ember-m3/adapters/m3-debug-adapter').default;
}
...
if (DEBUG) {
  for (let i = 0; i < 2; i++) {
    module(
      `integration/m3-debug-adapter ${i ? 'old hooks' : 'computeAttribute'}`,
      function (hooks) {
        setupTest(hooks);
        ...
        test('getModelTypes returns a list of model types', function (assert) {
          ...
          assert.deepEqual(
            modelTypes,
            modelTypeArray,
            'Correct list of m3 model types is returned'
          );
        });
