...
test('payloads can be rendered as m3 models', function(assert) {
  ...
  andThen(() => {
    ...
    assert.deepEqual(page.books().map(x => x.id()), [
      'isbn:9780760768570',
      'isbn:9780760768587',
      'isbn:9780760768594',
      'isbn:9780297609568'
    ...
    assert.deepEqual(page.books().map(x => x.authorName()), [
      'Winston Churchill',
      'Winston Churchill',
      'Winston Churchill',
      'Winston Churchill',
    ...
    assert.deepEqual(page.books().map(x => x.comments().map(x => x.body())), [
      ['This book is great', 'I agree'],
      [],
      ['', 'Yup'],
      [],
    ], 'able to read attributes through reference arrays');

    assert.deepEqual(page.books().map(x => x.comments().map(x => x.parts())), [
      [[], []],
      [],
