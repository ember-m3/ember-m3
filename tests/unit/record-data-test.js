...
import { recordDataToRecordMap } from 'ember-m3/utils/caches';
...
function computeNestedModel(key, value) {
  if (value !== null && typeof value === 'object') {
    return {
      id: key,
      type: 'com.exmaple.bookstore.book',
      attributes: value,
    };
  }
}
...
for (let testRun = 0; testRun < 2; testRun++) {
  module(
    `unit/record-data with  ${testRun === 0 ? 'old hooks' : 'computeAttribute'}`,
    function (hooks) {
      setupTest(hooks);
      ...
      test(`.eachAttribute callback fires only once per attribute name`, function (assert) {
        ...
        assert.deepEqual(attributeCallbackCounters, {
          localAttr: 1,
          dataAttr: 1,
          inFlightAttr: 1,
        });
      ...
      test(`.getServerAttr returns the server state`, function (assert) {
        ...
        assert.equal(
          recordData.getServerAttr('serverAttr'),
          'server',
          'server attr read correctly'
        );
      });
      ...
      test('.rollbackAttributes does not call notifyPropertyChange with undefined without hasChangedAttributes', function (assert) {
        ...
        assert.equal(
          ...
          0,
          'rollbackAttributes was not called'
        );
      });
      ...
      test('projection recordData connects with base recordData when committed with id', function (assert) {
        ...
        assert.equal(
          baseRecordData.id,
          '1',
          'Expected base recordData to have picked up the new ID'
        );
